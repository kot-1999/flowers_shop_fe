...
export async function GET() {
    try {
        ...
        return NextResponse.json({ message: error?.message ?? 'Failed to get cookie', status: 500 });
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ message: error?.message ?? 'Failed to set cookie', status: 500 });
    }
}